...
function leaveShowFun(param) {
    ...
    leaveShow.to('.cardInner', {
        ...
        if(leafTL==undefined){
            leafTL= gsap.to('.cardInner .leaf1,.cardInner .leaf2,.cardInner .leaf3,.cardInner .leaf4,.cardInner .leaf5', {
                duration: 65,
...
function enterShowFun(tl = null) {
    ...
    enterShow.to('.cardInner', {
        ...
        enterShow.fromTo('.cardInner .card .back .back-content .back-item-img', 
            { 
                opacity: 0, 
                scale: 8, 
                filter: 'blur(10px)' 
...
function enterBannerSmall() {
    ...
    let enterShow = gsap.timeline();
    enterShow
        .to('.cardInner', { maskSize: isPhone ? '145vw' : '62vw', duration: sessionStorage['index']!=='true' ? 0.5 : 0.6, ease: "power2.out", })
        .to('.cardInner .bulge', { opacity: 0, filter: 'blur(15px)', duration: sessionStorage['index']!=='true' ? 0.5 : 0.8, }, '<')
        .to('body', {
             height: 'auto', 
             overflow: 'auto', 
             ...
             onComplete(){
                ...
                if(window.scrollY==0){
                    ...
                    let delay_scroll= setTimeout(() => {
                                        gsap.to(window, { duration: 1.5, scrollTo:{y: scrollTo_id, offsetY: 70} });
                                    }, 500);
                }
             }
...
function innerScroll() {
    ...
    f_move.to('.cardInner.mask_div', { maskSize: '0vw', duration: 1, ease: 'power3.inOut' })
        .to('.cardInner .blur-3', { x: '-10vw', duration: 1, ease: 'power3.inOut' }, '<')
        .to('.cardInner .back-content .back-item-img', { y: '50vw', scale: 2, duration: 1, ease: 'power3.inOut' }, '<')
...
function phoneCardMove() {
    ...
    scrollCard.addEventListener('touchstart', function (event) {
        // 记录触摸起始点的坐标
        let nowX=parseFloat(gsap.getProperty(".scroll-card-box", "x", "vw").slice(0, -2));
        scrollX=nowX;
...
function phoneCardSwitch() {
    ...
    $('.navigation_box .prev_btn').click(function (e) { 
        ...
        gsap.to('.scroll-card-box', { x: `${card_scroll[card_index]}vw`, duration: 1, ease: 'power2.out' });
    ...
    $('.navigation_box .next_btn').click(function (e) { 
        ...
        card_index= card_index >= card_scroll.length-1 ? card_scroll.length-1 : card_index + 1;
        gsap.to('.scroll-card-box', { x: `${card_scroll[card_index]}vw`, duration: 1, ease: 'power2.out' });
        // gsap.to('.contanier', { backgroundPosition: `${card_scroll[card_index]}vw 0`, duration: 1.3, ease: 'power2.out' });
    });
...
barba.init({
    ...
    transitions: [{
        ...
        async before(data) {
            ...
            if ((data.current.url.path.indexOf('index') != -1 || data.current.url.path == '/') && (data.next.url.path.indexOf('activity')==-1 && data.next.url.path.indexOf('contact')==-1) ) {
