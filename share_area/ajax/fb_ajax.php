    ...
    switch ($_POST['type']) {
        ...
        case 'fb_signup':
            $member1=$pdo->select("SELECT m.Tb_index 
                                   ...
                                   WHERE ml.lgn_id=:lgn_id", ['lgn_id'=>$_POST['mem_fb_id']], 'one');

            if(!empty($member1['Tb_index'])){
                echo json_encode(['success'=>false, 'msg'=>'此FB已註冊過!']);
            }
            else{
        ...
        case 'fb_singin':
            ...
            if(!empty($member['Tb_index'])){
                ...
                echo json_encode(['success'=>false, 'msg'=>'此FB帳戶尚未註冊!']);
            }
            break;
